        ...
        $(function() {
            $('.change-retries').click(function() {
                ...
                if (isNaN(retries)) {
                    ...
                    window.location.href = activitiCtx + '/management/job/change/retries/' + $(this).data('jobid') + "?retries=" + retries+"&revision="+$(this).data('revision');
                }
            });
        });
    </script>
...
<%--                         <li><a href="delete/${job.id}"><i class="icon-trash"></i>删除</a></li> --%>
                        <li><a href="javascript:void(0);" title="更改执行次数" data-jobid="${job.id}" data-revision="${job.revision}" class="change-retries"><i class="icon-wrench"></i>执行次数</a></li>
                    </ul>
                </div>
            </td>
        </tr>
...
<form id="confirmForm" name="confirmForm" action="">
	<input type="hidden" id="retries" name="retries"/>
